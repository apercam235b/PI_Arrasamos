import asyncio
import websockets
import logging
import os
import requests
import argparse
from servo import posicion1, posicion2
from take_photo import captura_camara
from take_pizarra import captura_pizarra

# Configuración del logging
logging.basicConfig(level=logging.INFO, filename='log.txt', format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)
base_url = 'http://localhost:5080'

# Función para manejar argumentos
def parse_args():
    parser = argparse.ArgumentParser(description="Script para manejar clases y tomar fotos.")
    parser.add_argument('clase', type=str, help='Nombre de la clase')
    return parser.parse_args()

# Función para manejar clientes
async def handle_client(websocket, path, clase):
    is_running = True
    while is_running:
        message = await websocket.recv()  # Espera un mensaje
        logger.info(f"Recibido: {message}")
        if message == "1":
            url = f"{base_url}/log"
            log_message = "Moviendo servo a pizarra 1"
            logger.info(log_message)
            data = {"message": log_message}
            requests.post(url, json=data)

            logger.info("Ejecutando script de pizarra 1")
            response = "Se ha movido la pizarra 1"
            posicion1()
        elif message == "2":
            url = f"{base_url}/log"
            log_message = "Moviendo servo a pizarra 2"
            logger.info(log_message)
            data = {"message": log_message}
            requests.post(url, json=data)

            logger.info("Ejecutando script de pizarra 2")
            response = "Se ha movido a pizarra 2"
            posicion2()
        elif message == "3":
            url = f"{base_url}/log"
            log_message = "Se ha iniciado la grabacion"
            logger.info(log_message)
            data = {"message": log_message}
            requests.post(url, json=data)

            os.system(f"python3 video.py /datos/{clase} &")
            logger.info("Ejecutando script para empezar grabación")
            response = "Se ha iniciado la grabación"
        elif message == "4":
            url = f"{base_url}/log"
            log_message = "Se ha detenido la grabacion"
            logger.info(log_message)
            data = {"message": log_message}
            requests.post(url, json=data)

            os.system(f"python3 video.py parar &")
            logger.info("Deteniendo la grabación")
            response = "Se ha detenido la grabación"
        elif message == "5":
            url = f"{base_url}/log"
            log_message = "Se ha tomado una foto"
            logger.info(log_message)
            data = {"message": log_message}
            requests.post(url, json=data)
            logger.info("se ha tomado una foto")

            captura_camara(f"/datos/{clase}")
            response = "Se ha tomado una foto"
        elif message == "6":
            url = f"{base_url}/log"
            log_message = "Iniciando la clase"
            logger.info(log_message)
            data = {"message": log_message}
            requests.post(url, json=data)

            ruta_recursos = f"/datos/{clase}"
            os.makedirs(ruta_recursos, exist_ok=True)
            response = "Se ha iniciado la clase"
        elif message == "7":
            url = f"{base_url}/log"
            log_message = "Termina la clase"
            logger.info(log_message)
            data = {"message": log_message}
            requests.post(url, json=data)

            logger.info("Finalizando la clase")
            response = "Se ha finalizado la clase"
            #is_running = False
            #await websocket.close()
            quit()

        elif message == "8":
            url = f"{base_url}/log"
            log_message = "Tomando foto a una pizarra"
            logger.info(log_message)
            data = {"message": log_message}
            requests.post(url, json=data)
            logger.info("Se ha tomado una foto a una pizarra")

            captura_pizarra(f"/datos/{clase}")
            response = "Se ha tomado una foto a una pizarra"


        else:
            logger.info(f"Estado desconocido recibido: {message}")
            response = "No reconozco este estado"
        await websocket.send(response)

# Función principal
async def main(clase):
    server = await websockets.serve(lambda ws, path: handle_client(ws, path, clase), "localhost", 8766)
    logger.info(f"Server started and listening on ws://localhost:8766 for class {clase}")
    await server.wait_closed()

# Punto de entrada del script
if __name__ == "__main__":
    args = parse_args()
    asyncio.run(main(args.clase))
